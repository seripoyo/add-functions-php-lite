...
// error_log('Debug: $admin_menu_path = ' . print_r($admin_menu_path, true));
...
?>
<link href="<?php echo PLUGIN_PATH; ?>css/dashicon.min.css" rel=" stylesheet" />
<style>
	.grid_header,
	.grid_form_contents_item {
		display: grid;
		grid-template-columns: 200px repeat(<?php echo count( $users ) + 1; ?>, 130px) 300px 300px;
		...
		<form id="custom-menu-form" method="post" action="options.php">
			...
			<div class="grid-container">
				<div class="grid_header">
					...
					<div class="grid-item label admin"><?php _e( 'Administrators only', 'add-functions-php' ); ?></div>
                    ...
                    <?php foreach ($labels as $id => $label) : ?>
                        ...
                        <div class="grid_form_contents_item">
							...
							<div class="grid-item admin toggle-container">
								<label class="toggle">
									<input class="toggle__input" type="checkbox" name="sidebar_menu_hidden_options[admin][<?php echo esc_attr( $id ); ?>]" 
																																					<?php
																																										$checked = isset( $selected_options['admin'][ $id ] ) ? $selected_options['admin'][ $id ] : '1';
																																										checked( $checked, '1' );
																																					?>
									role="switch">
